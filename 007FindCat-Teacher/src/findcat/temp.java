...
public class temp extends JFrame{
  ...
  ImageIcon bat=new ImageIcon("burger.gif");
  ...
  public temp() {
    setTitle("Find the Cat - Amaia Guerricabeitia 2018");
    setResizable(false);
    ...
    for (int j=0; j<row; j++) {
        for (int i=0; i<col; i++) {
            ...
            choiceLabel[col*j+i].setBackground(Color.BLACK);
  ...
  private void labelMouseClicked(MouseEvent e) {
    Component clickedComponent=e.getComponent();
        ...
        choiceLabel[choice].setBackground(Color.CYAN);
        if (choice==catlocation) {
          choiceLabel[choice].setIcon(cat);
          newButton.setEnabled(true);
          JOptionPane.showMessageDialog(null, "It took you " + point + " tries to find the chest");
          for (int i=0; i<difficulty; i++) {
